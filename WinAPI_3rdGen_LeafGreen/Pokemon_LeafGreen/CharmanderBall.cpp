...
ACharmanderBall::ACharmanderBall()
{
}
...
void ACharmanderBall::BeginPlay()
{
	...
	SelectImage->SetImage("Select_CBall.png");
...
void ACharmanderBall::Tick(float _DeltaTime)
{
	...
	if (true == BallCollision->CollisionCheck(ECollisionOrder::Player, Result))
	{
		...
		else if (true == PlayerHelper::PlayerPause && false == PlayerHelper::IsUseMenu)
		{
			if (true == UEngineInput::IsDown('P'))
			{
				...
				dynamic_cast<Pokemon3rdGen_Core*>(GEngine)->AddEntry(4, 5);
